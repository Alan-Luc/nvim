local exclude_tests_from_query = function (_, opts)
  local glob = "!**/*test*"
  local query = opts.last_query or ""
  if query:find(glob, 1, true) then
    return
  end

  local updated = query
  if updated:find(" -- ", 1, true) then
    updated = updated .. " " .. glob
  else
    updated = updated .. " -- " .. glob
  end

  opts.__call_fn(vim.tbl_deep_extend("force", opts.__call_opts or {}, {
      resume = true,
      search = vim.trim(updated),
      query = nil
    }))
end
      ...
      actions = {
        ...
        ["ctrl-b"] = {
          fn = function (selected, opts)
            ...
            vim.notify(string.format("Added %d file%s to buffers", count, count > 1 and "s" or ""), vim.log.levels.INFO)
          end,
          reload = true
        }
      }
